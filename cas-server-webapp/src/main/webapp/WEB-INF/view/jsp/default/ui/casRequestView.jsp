		...
		<div class="group">
			<c:choose>
				<c:when test="${not empty service and (fn:containsIgnoreCase(service, 'sales') or fn:containsIgnoreCase(service, 'advertisingdemos'))}">
					<input id="submit" class="btn btn-inverse" name="submit" value="Request"
					       tabindex="3" type="submit" disabled="disabled"/>
				</c:when>
				<c:otherwise>
					<input id="submit" class="btn btn-primary" name="submit" value="Request"
					       tabindex="3" type="submit" disabled="disabled"/>
				</c:otherwise>
			</c:choose>
	...
	<script>
		...
		collective.callback = function () {
			...
			var validate = function (event) {
				if (primary_email('#email') && primary('#name')) {
